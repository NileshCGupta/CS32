...
//  Hw4
//
//  Created by Nilesh Gupta on 8/3/15.
...
public:
	...
	// If there exists a single item that has the largest number of instances
	// in the multiset, then copy into value that item in the multiset and
	// return true.  However, if there exist more than 1 item that have the
	// largest number of instances in the multiset,  then do not copy into
	// value any item in the multiset and return false. In other words, value
	// should remain unchanged.  If there's no item in the multiset, return
	// false.
	
	bool getLargestValue(ItemType& value) const;
	// If there exists a value that is the largest value among all the values
	// in the multiset, then copy into value that item in the multiset and
	// return true; otherwise, return false.  For both unsigned long and
	// string data type, the higher value can be found by using greater than
	// operator (>).  For example, 100 is greater than 20, so 100 > 20 is
	// true.  "ZOO" is greater than "ABC", so "ZOO" > "ABC" is true.
	
	bool getSecondLargestValue(ItemType& value) const;
	// Similar to getLargestValue(), but this time you need to find the
	// second largest value.  If there exists a value that is the 2nd largest
	// value among all the values in the multiset, then copy into value that
	// item in the multiset and return true; otherwise, return false.  Please
	// note that you cannot use any sorting algorithm to sort the multiset.
...
template <typename ItemType>
bool Multiset<ItemType>::getMostFrequentValue(ItemType& value) const
{
	if (empty())
		return false;
	
	// Start by assuming first value is the unique most frequent value
	
	Node* pMostFrequent = m_head->m_next;
	bool uniqueMostFrequent = true;
	
	// See if any other value is the most frequent
	
	for (Node* p = pMostFrequent->m_next; p != m_head; p = p->m_next)
	{
		int diff = (p->m_count - pMostFrequent->m_count);
		if (diff == 0)  // most frequent value, but not unique
			uniqueMostFrequent = false;
		else if (diff > 0)  // a new unique most frequent value
		{
			pMostFrequent = p;
			uniqueMostFrequent = true;
		}
	}
	...
	if (uniqueMostFrequent)
		value = pMostFrequent->m_value;
	return uniqueMostFrequent;
}

template <typename ItemType>
bool Multiset<ItemType>::getLargestValue(ItemType& value) const
{
	if (empty())
		return false;
	
	// Start by assuming first value is the largest
	
	Node* pLargest = m_head->m_next;
	
	// See if any other value is larger
	
	for (Node* p = pLargest->m_next; p != m_head; p = p->m_next)
		if (p->m_value > pLargest->m_value)
			pLargest = p;
	value = pLargest->m_value;
	return true;
}

template <typename ItemType>
bool Multiset<ItemType>::getSecondLargestValue(ItemType& value) const
{
	if (m_uniqueSize < 2)
		return false;
	
	// pLargest1 points to largest seen so far
	// pLargest2 points to second largest seen so far
	// Start by assuming they're in first two positions.
	
	Node* pLargest1 = m_head->m_next;
	Node* pLargest2 = pLargest1->m_next;
	Node* third = pLargest2->m_next;
	
	if (pLargest1->m_value < pLargest2->m_value)
		std::swap(pLargest1, pLargest2);
	...
	for (Node* p = third; p != m_head; p = p->m_next)
	{
		...
		if (v > pLargest1->m_value)  // largest value so far?
		{
			pLargest2 = pLargest1;  // old largest now second largest
			pLargest1 = p;
		}
		else if (v > pLargest2->m_value)  // second largest?
			pLargest2 = p;
	}
	
	// Set value to second largest
	
	value = pLargest2->m_value;
	return true;
}
...
template <typename ItemType>
typename Multiset<ItemType>::Node* Multiset<ItemType>::insertHelper(const ItemType& value, int nToInsert)
{
	Node* p = find(value);
...
template <typename ItemType>
void uniqueIntersect(const Multiset<ItemType>& m1, const Multiset<ItemType>& m2, Multiset<ItemType>& result)
{
	Multiset<ItemType> res;
